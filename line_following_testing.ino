...
//7V
int recoverSpeed = 250;
int recoverReverseSpeed = 215;
...
// //6.6V
// int recoverSpeed = 240;
// int recoverReverseSpeed = 210;
...
void recvMsg(uint8_t *data, size_t len) {
  ...
  // -------- CHANGE BASE SPEED --------
    ...
    int newBase = message.substring(6).toInt();
    baseSpeed = newBase;
  ...
  // -------- CHANGE RECOVER SPEEDS --------
  else if(message.startsWith("RECSPEED")){
    int newRecoverSpeed = message.substring(9).toInt();
    recoverSpeed = newRecoverSpeed;

    WebSerial.print("New Recover Speed: ");
    WebSerial.println(recoverSpeed);
  }

  else if(message.startsWith("REVSPEED")){
    int newRecoverReverseSpeed = message.substring(9).toInt();
    recoverReverseSpeed = newRecoverReverseSpeed;

    WebSerial.print("New Recover Reverse Speed: ");
    WebSerial.println(recoverReverseSpeed);
  }
  ...
  // -------- SHOW CURRENT VALUES --------
    ...
    WebSerial.print("Recover Speed: ");
    WebSerial.println(recoverSpeed);

    WebSerial.print("Recover Reverse Speed: ");
    WebSerial.println(recoverReverseSpeed);
  }
...
void loop() {
  ...
  if(!lineDetected){
    ...
    if(lastDirection >= 0){
      analogWrite(IN1,recoverReverseSpeed);
      ...
      digitalWrite(IN2,LOW);
      analogWrite(IN3,recoverReverseSpeed);
  ...
  int adjust = correction * 200;

  int speedLeft  = dynamicBaseSpeed - adjust;
  int speedRight = dynamicBaseSpeed + adjust;

  speedLeft  = constrain(speedLeft,120,255);
  speedRight = constrain(speedRight,120,255);
